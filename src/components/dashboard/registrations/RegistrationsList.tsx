...
export default function RegistrationsList() {
  ...
  // Filtrage des inscriptions avec useMemo pour optimiser les performances
  ...
  // Gestion de la sélection d'une inscription
  ...
  // Gestion de la sélection de toutes les inscriptions
  ...
  const handleDownloadTicket = async (registrationId: string) => {
    try {
      // Simule le début du téléchargement
      toast.loading('Préparation du téléchargement...');
      
      // Remplacer par l'API réelle
      // const response = await registrationsAPI.downloadTicket(registrationId);
      
      // Simuler un téléchargement réussi après 1.5 secondes
      setTimeout(() => {
        toast.dismiss();
  ...
  // Obtenir la date relative pour l'événement
                    ...
                    <div className="ml-4 flex-1 min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap items-center text-xs text-gray-500 gap-x-4 gap-y-1">
                        <div className="flex items-center">
                          ...
                          <span className="ml-1 text-gray-400">({getRelativeDate(event.start_date)})</span>
                        </div>
                        ...
                        {registration.tickets && registration.tickets.length > 0 && (
                          <div className="flex items-center">
                            ...
                            <span>
                              {registration.tickets.reduce((total, ticket) => total + ticket.total_price, 0).toLocaleString()} XAF
                            </span>
                          </div>
                        )}
                      </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-8 text-center">
          ...
          {!(searchTerm || filter !== 'all') && (
            <div className="mt-6">
              ...
              >
                <Ticket className="mr-2 -ml-1 h-5 w-5" />
